...
import {
  BootNotification,
  STATUS_ACCEPTED,
} from "../src/commands/BootNotification";
...
describe("onMessage", () => {
  test("Send response message", async () => {
    ...
    const message =
      '[2, "1", "BootNotification", {"chargePointVendor":"BrandX","chargePointModel":"ModelY"}]';
    ...
    expect(conn.sendMessage).toBeCalledWith(
      "1",
  ...
  test("Error when wrong command", () => {
    ...
    expect(conn.onMessage(message)).rejects.toThrow(
      'Failed to parse message: "[3,", Unexpected end of JSON input'
    );
  });
  ...
  test("Error when response without request", () => {
    const conn = new Connection(socketEmpty);
    const message =
      '[3,"1","BootNotification",{"chargePointVendor": "BrandX"}]';

    expect(conn.onMessage(message)).rejects.toThrow(
      "Response for unknown message 1"
    );
  });
  ...
  test("Error when response without request", () => {
    ...
    expect(conn.onMessage(message)).rejects.toThrow(
      'Invalid value "123" for field chargePointVendor'
    );
  });
});
...
describe("sendMessage", () => {
  test("Send response message", async () => {
    ...
    expect(socket.send).toBeCalledWith(
      expect.stringMatching(
        /\[3,"(.*)",{"status":"Accepted","currentTime":"(.*)","interval":60}\]/
      )
    );
  });
  ...
  test("Send message and wait for response", async () => {
    ...
    expect(socket.send).toBeCalledWith(
      `[2,"${messageId}","BootNotification",{"chargePointVendor":"BrandX","chargePointModel":"ModelY"}]`
    );
